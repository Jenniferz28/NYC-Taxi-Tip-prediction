...
'''2. Plot a histogram of Trip Distance
 Apparently the most of trip_dis cumulates below 100. But it is not quite
straightforward to see the pattern. Let’s show kernel density estimate: 
The histogram and frequency plot tells us that the trip distance is rightskewed distribution, 
Mean distance is larger than median distance. This
indicates that appropriate transformation of this variable (for example, BoxCox transformation)
 is needed if we are using linear model'''
...
''' According to the 4 boxplots above, apparently at JFK area, the payment type
has huge impact on the Tip_amount while the Fare_amount is relatively
stable on payment type. The RateCodeID also makes the Tip_amount
selective. Furthermore, the Tip_amount is dependent on the hour_drop ( or
hour_pick) . The next is to look at the possible relationship between Tip_amount and
travel distance. Overall, the Fare_amount and Tip_amount are positively
increasing as the Trip_distrance increases. Which agree with common sense
that people would spend more and tip more if they take longer-distance trip'''
...
'''Most of the transactions are paid by credit card or cash. The transactions
Payment_type=1 has broad distribution, while Payment_type=2 looks like
only at one value. Let’s verify our hypothesis by plotting the individual
histogram. Tip by cash payment is mostly 0, which makes
sense because it was not recorded by the meter. For other payment types,
it seems like the Tip_p is very likely to be 0 '''
...
'''The weekly average speed are the same. A multi-sample ANOVA could be
used to test if the weekly average speed are equal. Test for normal
distribution is needed before the ANOVA'''
